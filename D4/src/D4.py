...
class D4():
    ...
    def minmax_depth(self, depth, blur=True):
        ...
        if blur:  # use median filter to filter out peak values for visualization.
            depth = F.pad(depth,[4,4,4,4],'reflect')
            # Try the modern API format (for newer Kornia versions)
            try:
                depth = kornia.filters.median_blur(depth,(9,9))
            except AttributeError:
                # Fallback
                try:
                    median_blur = kornia.filters.MedianBlur((9,9))
                    depth = median_blur(depth)
                except:
                    # Last resort, use OpenCV instead
                    import cv2  # Only import cv2 here
                    depth_np = depth.squeeze().cpu().numpy()
                    depth_np = cv2.medianBlur(depth_np, 9)
                    depth = torch.from_numpy(depth_np).unsqueeze(0).unsqueeze(0).to(depth.device)
            depth = depth[:,:,3:h-3,3:w-3]
